...
Yomi-Search風のディレクトリ型サーチエンジンです。同人サイト向けに設計されており、検索避け対応、簡単設置、軽量動作を重視しています。
...
各PHPファイルの `<style>` タグ内でCSSを編集：

```css
/* メインカラーの変更 */
.header { background: linear-gradient(135deg, #your-color1, #your-color2); }
.search-box input[type="submit"] { background: #your-color; }
```
...
将来的に複数のサーチエンジンを簡単に作成できるよう、以下の拡張を予定：

1. **設定ファイルテンプレート**の自動生成
2. **ワンクリックインストーラー**の提供
3. **テーマシステム**の実装
4. **プラグインシステム**の実装
